...
def bowtie2_align(bt2_db_dir, bt2_db_name, bamfile_path, args):
    ...
    try:
        bt2_command = f"bowtie2 -x {bt2_db_prefix} {max_fraglen} {max_reads} --{aln_mode} --{aln_speed} --threads {args.num_cores} -q {r1} {r2}"
        command(f"set -o pipefail; {bt2_command} | \
            tee >(samtools fastq -F 2 -1 unmapped_1.fastq -2 unmapped_2.fastq -) >(samtools view --threads {args.num_cores} -b -f 2) >/dev/null | \
